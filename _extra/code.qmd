...
continent_count <- places |>
  group_by(continent) |>
  summarise(countinent_ufo_sighting = n()) |>
  ungroup() |>
  arrange(desc(countinent_ufo_sighting))
...
country_count <- places |>
  group_by(country_code, continent, full_countryname) |>
...
country_count$patching <- paste("world", country_count$continent, 
                                country_count$full_countryname,
                                country_count$country_ufo_sighting,
                                sep = "/")

counting <- as.Node(country_count, pathName = "patching")

circular_plot <- circlepackeR(counting, size = "country_ufo_sighting", 
                              color_min = "hsl(202,80%,80%)", color_max = "hsl(341,30%,40%)", 
                              width = 3000)

title <- "Occurence of UFO Sightings across the cities in Each Region"

final_style <- paste("color:","rgb(93, 63, 211)" , ";",
                     "font-size", "14px", ";", 
                     "text_aling: centre;",
                     "font-weight bold:")
